...
async function main() {
    ...
    // Also check if system user has IG permissions
    console.log('\n=== SYSTEM USER IG PERMISSIONS ===');
    const res = await fetch(`${BASE}/${accountId}/instagram_accounts?access_token=${token}`);
    const data = await res.json();
    console.log('Ad acct IG:', JSON.stringify(data, null, 2));
