...
async function main() {
    ...
    // First, search for the correct geo key for Itamaraju
    console.log('=== BUSCANDO GEO TARGETING ===');
    const geoRes = await gql(`search?type=adgeolocation&q=Itamaraju&location_types=["city"]`, token);
    console.log('Resultados geo:', JSON.stringify(geoRes.data, null, 2));

    const targetingConfigs = [
        {
            name: 'Cidade sem raio',
            targeting: {
        ...
        {
            name: 'Cidade com raio 10mi (default)',
            targeting: {
                geo_locations: { cities: [{ key: '255700', radius: 10, distance_unit: 'mile' }] },
                age_min: 28, age_max: 60,
            }
        },
        {
            name: 'Custom location lat/lng',
            targeting: {
                geo_locations: { custom_locations: [{ latitude: -17.03, longitude: -39.53, radius: 15, distance_unit: 'kilometer' }] },
                age_min: 28, age_max: 60,
            }
        },
        {
            name: 'Country BR + region',
            targeting: {
    ...
    let adSetId;
    for (const cfg of targetingConfigs) {
        console.log(`\n--- ${cfg.name} ---`);
        try {
    ...
    if (!adSetId) {
        console.log('\nTodas configs de targeting falharam!');
        await db.$disconnect();
        return;
    }
    ...
    if (adsCreated > 0) {
        ...
        try { await post(adSetId, token, { status: 'ACTIVE' }); console.log('  Ad Set ATIVO'); } catch (e) { console.log(`  AdSet err`); }
        try { await post(campaignId, token, { status: 'ACTIVE' }); console.log('  Campanha ATIVA'); } catch (e) { console.log(`  Campaign err`); }
    }
